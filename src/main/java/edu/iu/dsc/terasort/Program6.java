...
//    localRank = 0;
        ...
        for (int procFileNo = 0; procFileNo < filesPerProcess; procFileNo++) {
            ...
            if (sendBuffer == null) {
                sendBuffer = MPI.newByteBuffer((int) (records.length * 4L / worldSize));
                receiveBuffer = MPI.newByteBuffer((int) (records.length * 4L / worldSize));
            }
            ...
            for (int i = 1; i < worldSize; i++) {
                ...
                if (i % (worldSize / 2) == 0) {
                    LOG.info(String.format("Rank %d file %d progressing by sending to %d and receiving %d", rank, procFileNo, sendingRank, receivingRank));
                }
            }
        ...
        long sortTimeinner = sorter.merge();
        MPI.COMM_WORLD.barrier();
