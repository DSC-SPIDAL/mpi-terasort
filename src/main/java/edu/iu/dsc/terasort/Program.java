...
public class Program {
  ...
  public void partialSendExecute() throws MPIException {
    ...
    // we are going to use prealloacted worldSize buffers to send the correct values
    byte[][] sendBuffer = new byte[worldSize][];
    for (int i = 0; i < sendBuffer.length; i++) {
      sendBuffer[i] = new byte[sendBufferSize * Record.RECORD_LENGTH];
    }
    ...
    // now go through the partitions and send add them to correct sendbuffer
    for (int i = 0; i < numberOfRecords; i++) {
      System.arraycopy(keyBuffer, 0, records, i * Record.RECORD_LENGTH, Record.KEY_SIZE);

      Text key = new Text(keyBuffer);
      Record r = new Record(key);
      int partition = partitionTree.getPartition(r.getKey());
//      LOG.log(Level.INFO, "Partition is: " + partition);

      byte []data = sendBuffer[partition];
      ...
      LOG.info("Send buffer size rank: " + partition + " size: " + sendBufferSizes[partition]);
    }
  ...
  private void sendBuffer(MergeSorter sorter, int size,
                          int partition, byte[] data) throws MPIException {
    int[] sizeBuffer = new int[1];
    if (partition != rank) {
      sizeBuffer[0] = size;
      // LOG.info("Sending size: " + sizeBuffer[0] + " to partition: " + partition + " from: " + rank);
      MPI.COMM_WORLD.send(sizeBuffer, 1, MPI.INT, partition, 100);
      // it is time to send this buffer
      LOG.info("Sending actual data of size: " + size + " to: " + partition + " from: " + rank);
      MPI.COMM_WORLD.send(data, size, MPI.BYTE, partition, 100);
    } else {
  ...
  private class ReceiveWorker implements Runnable {
    ...
    private int[] sizeBuffer = new int[1];

    public ReceiveWorker(MergeSorter sorter, int worldSize, int bufferSize) {
      this.sorter = sorter;
      this.finishedSenders = worldSize;
    }

    @Override
    public void run() {
      while (true) {
        if (finishedSenders == worldSize - 1) {
          break;
        }
        ...
        for (int i = 0; i < worldSize; i++) {
          if (i == rank) continue;
          try {
//            LOG.info("Calling iprobe");
            Status status = MPI.COMM_WORLD.iProbe(i, 100);
            if (status == null) {
              continue;
            }
            int size = status.getCount(MPI.INT);
            if (size > 0) {
              // first receive the size, this is used to mark then end of receives from the source
              MPI.COMM_WORLD.recv(sizeBuffer, size, MPI.INT, status.getSource(), 100);
              if (sizeBuffer[0] == 0) {
                LOG.info("Nothing more to receive from: " + status.getSource() + " rank: " + rank);
                finishedSenders++;
                continue;
              }
              byte[] data = new byte[sizeBuffer[0]];
              // LOG.info("Calling second recv: " + sizeBuffer[0] + " rank: " + rank + " source: " + status.getSource() + " tag: " + status.getTag());
              LOG.info("Calling second recv_actual: " + sizeBuffer[0] + " rank: " + rank + " source: " + i + " tag: " + 100);
              MPI.COMM_WORLD.recv(data, sizeBuffer[0], MPI.BYTE, i, 100);
              sorter.addData(i, data);
            }
          } catch (MPIException e) {
            e.printStackTrace();
          }
